...
class BalloonCrawl(CrawlSpider):
    ...
    def parse(self, response):
        ...
        elif 'extractor_rules' in config.output and (get_attr(self.resolved, "extractor_rules",False) is False or response.url in self.start_urls) :
    ...
    def start_download(self, html, source):
        ...
        if contains(config.levels, '1'):
            ...
            if isinstance(config_level, dict):
                href = self.process_extract_data(None, html, config_level)
            elif isinstance(config_level, unicode) or isinstance(config_level, str):
                href = extract_data(xpath(html, config_level))
            if href:
                href = href[0] if isinstance(href, list) else href
                request = Request(urljoin(self.base_url, href), callback=self.parse_product)
                return self.process_request(request, html, source)
            else:
                log.msg("The course link must be not null", level=log.ERROR)
                return None
        elif html and ( isinstance(html, unicode) or isinstance(html, unicode)):
    ...
    def is_finish_download(self, index, response):
        ...
        for level, value in config.levels.items():
            if not level in product_cache.keys():
                return False
    ...
    def closed(self, reason):
        from scrapy_bank.selenium_api import driver
        try:
            driver.close()
...
class ConfigFile:
    def __init__(self, config=None, config_file_path=None):
        ...
        self.base_url = get_attr(config, 'base_url')
        self.output =  get_attr(config, 'output_config')
        self.levels = config['levels']
        self.pre_filters = get_attr(self.output, 'pre_filters')
        self.post_filters = get_attr(self.output, 'post_filters')
        self.post_interceptors = get_attr(self.output, 'post_interceptors')
